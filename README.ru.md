...
От чьего имени публикует Studio, обслуживает ли она публичный сайт, её часовой пояс и параметры видео лежат в её собственной базе и читаются/меняются через `bun run --filter @alexgetman/backend ops -- studio-profile`. Ключи остаются в игнорируемом `apps/backend/secrets.env`, а площадки — в реестре каналов. Публикация текста, видео и сбор аналитики работают всегда.
