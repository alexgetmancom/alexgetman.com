...
Временные медиафайлы, которые внешняя площадка забирает во время публикации, складываются в `/data/site/media/staging`. Контейнер сам создаёт этот каталог и исправляет владельца до сброса root-прав. Маршрут `/media/staging/` доступен даже при выключенном публичном сайте, поэтому для свежего self-host не нужны ручные `mkdir` и `chmod`.
