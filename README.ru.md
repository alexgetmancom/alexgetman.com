...
Скрипт останавливается до того, как что-то изменит, если нет Docker или домен не резолвится в эту машину, — это две причины, по которым установка выглядит успешной, а система не работает. Дальше он сам пишет `.env` со сгенерированными тремя секретами, поднимает стек, ждёт `/readyz` через Caddy и печатает адрес Command Center вместе с токеном. Повторный запуск обновляет установку: существующий `.env` остаётся нетронутым.

Caddy сам получает и продлевает TLS-сертификат, поэтому ни certbot, ни таймер продления настраивать не нужно. Ссылка на Command Center, которую напечатал установщик, входит с `COMMAND_CENTER_TOKEN` из `.env`: пустая Studio откроется тремя шагами до первого черновика, а не пустой аналитикой.

Публичный сайт по умолчанию выключен — большинству нужен конвейер публикации в уже существующие каналы, а не ещё один сайт, за которым надо следить. Включите его через operations CLI внутри контейнера:

```bash
docker compose exec app bun /app/ops/cli.js studio-profile-set --site-enabled
```

Сайт с лентами, sitemap и Markdown-эндпоинтами появится по адресу `https://ваш-домен/` со следующего запроса.
...
Порты публикует только Caddy; приложение доступно исключительно через него. Больше ничего в `.env` для запуска не требуется — Studio без credentials площадок обслуживает Command Center и никуда не публикует. Добавьте Telegram или MCP как интерфейс автора, затем подключайте назначения в Command Center → Studio; `docker compose exec app bun /app/ops/cli.js doctor` покажет, чего не хватает каждому включённому назначению.

`docker compose exec app bun /app/ops/cli.js studio-profile` показывает, от чьего имени публикует эта Studio, её часовой пояс, наличие публичного сайта и тайминги видео; `studio-profile-set` в той же CLI их меняет. Они лежат в базе, поэтому переживают передеплой и не требуют рестарта.

Перед обновлением `docker compose exec app bun /app/ops/cli.js status` показывает текущий `gitRevision`. `latest` следует за revision, работающей в production автора; чтобы оставить проверенную установку неизменной, укажите в `.env` `SOLO_PUBLISHER_IMAGE=ghcr.io/alexgetmancom/solo-publisher:<полный-gitRevision>`. Обновление образа — `docker compose pull && docker compose up -d`, диагностика — `docker compose logs -f app`.

Две вещи, которые стоит знать сразу. Когда настроен Telegram, Studio ежедневно и беззвучно присылает копию
...
больше того, что принимает Telegram. Для них есть отдельная команда:
`docker compose exec app bun /app/ops/cli.js backup-media` складывает видео,
постеры, story-карточки и файлы сайта в `BACKUP_DIR_HOST`, который не должен
находиться на том же томе, что и защищаемые данные. Запускайте её по
расписанию: `doctor` считает деплой нездоровым, пока там нет архива свежее
недели. Ежедневную копию базы можно отключить в Настройках → Уведомления →
Бэкап базы. И Telegram не отдаёт файлы больше
...
От чьего имени публикует Studio, обслуживает ли она публичный сайт, её часовой пояс и параметры видео лежат в её собственной базе и читаются/меняются через `bun run --filter @solo-publisher/backend ops -- studio-profile`. Ключи остаются в игнорируемом `apps/backend/secrets.env`, а площадки — в реестре каналов. Публикация текста, видео и сбор аналитики работают всегда.
