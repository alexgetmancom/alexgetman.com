...
`studio.yaml` — это версионируемый файл включения возможностей без секретов. Ключи остаются в игнорируемом `apps/backend/secrets.env`. Текстовая Studio может обслуживать сайт и Telegram без настройки видеоплатформ; для отдельного видеоканала есть пример `studio.video-only.example.yaml`.
