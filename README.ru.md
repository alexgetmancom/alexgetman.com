...
[Рабочий сайт](https://alexgetman.com/ru/) · [Быстрый запуск](#быстрый-запуск) · [Архитектура](#как-это-работает)
...
Скопируйте шаблон секретов и выберите только те модули, которыми будете пользоваться:

```bash
cp apps/backend/secrets.env.example apps/backend/secrets.env
```

`studio.yaml` — это версионируемый файл включения возможностей без секретов. Ключи остаются в игнорируемом `apps/backend/secrets.env`. Публикация текста, видео и сбор аналитики работают всегда; Studio объявляет только свой публичный сайт и видеоплатформы, куда публикует. Пример второй Studio — `studio.maru.example.yaml`.
...
Доступность зависит от модулей и ключей, включённых в конкретном deployment. Solo Publisher использует ваши собственные аккаунты и API credentials и не становится посредником между вами и аудиторией.
...
- MCP поверх HTTP
- ffmpeg и sharp
- Docker Compose, nginx и deployment неизменяемых образов
