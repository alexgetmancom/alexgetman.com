...
Две половины: Studio должна знать, что площадка есть, и держать ключи к ней. Они
разделены намеренно — никто не спрашивает ключи к площадке, куда вы не публикуете.
...
`doctor` называет ровно те настройки, которых площадке не хватает, и никогда не
печатает те, что уже есть. Впишите их в `.env`, перезапустите, повторите. Те же
два шага доступны из Telegram-бота и из MCP-клиента.

**Instagram и Threads.** Публиковать в Meta напрямую — это своё приложение,
Professional-аккаунт и привязанная страница Facebook. Токен Threads вдобавок истекает через 60 дней
после выпуска, и никто здесь его не продлевает — публикация встанет, пока вы не
выпустите новый. Подключение через [Zernio](https://zernio.com) заменяет всё это
одним `ZERNIO_API_KEY` — и для ленты, и для Threads, и для Stories, потому что
канал несёт в себе провайдера, через которого доставляется, а бот
покажет ваши аккаунты списком в Настройки → Каналы. Нативный путь остаётся, если
предпочитаете держать собственные токены.

**YouTube.** Создайте проект в Google Cloud и OAuth-клиент типа *TV and Limited
Input devices*, впишите его id и secret в `.env`, затем:

```bash
docker compose exec app bun /app/ops/cli.js youtube-authorize --locale ru
```

Команда напечатает короткий код и адрес. Введите код на любом устройстве с
браузером, подтвердите — и она напечатает refresh-токен для `.env`. У каждой
Studio свой проект Google, потому что квота YouTube считается на проект, а не на
пользователя.

**Telegram** требует только тот токен бота, из которого вы уже пишете.
