...
Caddy сам получает и продлевает TLS-сертификат, поэтому ни certbot, ни таймер продления настраивать не нужно. Через минуту Command Center доступен по адресу `https://ваш-домен/command-center`. Войдите с `COMMAND_CENTER_TOKEN` из `.env`: пустая Studio откроется тремя шагами до первого черновика, а не пустой аналитикой.
