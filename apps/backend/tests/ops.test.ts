...
describe("TypeScript operations tooling", () => {
  it("creates a consistent SQLite backup", async () => {
    ...
    try {
      ...
      expect(backendDb.sqlite.prepare("SELECT backup_path FROM deployment_snapshots ORDER BY id DESC LIMIT 1").get()).toEqual({ backup_path: backup });
    } finally {
      backendDb.close();
    }
  });
  ...
  it("plans and applies a metrics backfill under a maintenance lock", () => {
    ...
    try {
      ...
      backendDb.sqlite.prepare("INSERT INTO posts(post_key,post_id,channel,message_id,date_utc,status,created_at,updated_at) VALUES ('post:1',1,'test',1,?,'active',?,?)").run(now, now, now);
      backendDb.sqlite.prepare("INSERT INTO post_targets(post_key,target,status,updated_at) VALUES ('post:1','devto','published',?)").run(now);
      ...
      expect(backendDb.sqlite.prepare("SELECT check_count,frozen_at FROM metric_schedule WHERE post_key='post:1' AND target='devto'").get()).toEqual({ check_count: 0, frozen_at: null });
