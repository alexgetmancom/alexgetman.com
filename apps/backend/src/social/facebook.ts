...
export async function publishToFacebook(payload: Record<string, unknown>, config: BackendConfig, fetchImpl: typeof fetch = fetch): Promise<PublishResult> {
  ...
  if (media.some((item) => item.type === "VIDEO")) {
    ...
    const res = await callFacebook(config, `${config.FACEBOOK_PAGE_ID}/videos`, { file_url: video.vpsUrl, description: text }, fetchImpl, true);
  ...
  if (media.length > 0) {
    ...
    const res = await callFacebook(config, `${config.FACEBOOK_PAGE_ID}/feed`, { message: text, attached_media: JSON.stringify(photoIds.map((id) => ({ media_fbid: id }))) }, fetchImpl);
...
async function callFacebook(config: BackendConfig, endpoint: string, payload: Record<string, unknown>, fetchImpl: typeof fetch, video = false): Promise<FacebookResponse> {
