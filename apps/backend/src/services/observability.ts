...
export async function runObservabilityCycle(
  ...
  for (const event of events) {
    const key = crypto
      .createHash("sha256")
      .update(`${event.event_type}\0${event.target ?? ""}\0${event.message}`)
      .digest("hex");
    const dedup = backendDb.sqlite.prepare("SELECT last_sent_at,suppressed_count FROM alert_dedup WHERE alert_key=?").get(key) as
      | { last_sent_at?: string; suppressed_count?: number }
      | undefined;
    ...
    if (bot && config.ADMIN_IDS[0]) {
      await bot.api.sendMessage(
        config.ADMIN_IDS[0],
        `[${event.severity.toUpperCase()}] ${event.target ?? event.event_type}\n${event.message}`.slice(0, 4000),
      );
      alerts += 1;
      const now = new Date().toISOString();
      backendDb.sqlite
        .prepare(`INSERT INTO alert_dedup(alert_key,last_sent_at,suppressed_count) VALUES (?,?,0)
        ON CONFLICT(alert_key) DO UPDATE SET last_sent_at=excluded.last_sent_at,suppressed_count=0`)
        .run(key, now);
...
function scanPublicationFailures(config: BackendConfig, backendDb: BackendDb): void {
  ...
  const stale = backendDb.sqlite
    .prepare("SELECT job_id,post_key,target,locked_at FROM publish_jobs WHERE status='publishing' AND locked_at<?")
    .all(staleBefore) as Array<{ job_id: number; post_key: string | null; target: string; locked_at: string }>;
  const failed = backendDb.sqlite
    .prepare("SELECT post_key,target,last_error FROM publish_jobs WHERE status='failed' ORDER BY updated_at DESC LIMIT 100")
    .all() as Array<{ post_key: string | null; target: string; last_error: string | null }>;
  const insert = backendDb.sqlite.prepare(
    "INSERT INTO post_events(post_key,event_type,severity,target,message,details_json,created_at) SELECT ?,?,?,?,?,?,? WHERE NOT EXISTS (SELECT 1 FROM post_events WHERE post_key IS ? AND event_type=? AND target IS ?)",
  );
  backendDb.sqlite.transaction(() => {
    for (const job of stale)
      insert.run(
        ...
        "error",
        job.target,
        `Publish job ${job.job_id} exceeded lock timeout`,
        JSON.stringify({ job_id: job.job_id, locked_at: job.locked_at }),
        now,
        job.post_key,
        "queue.stale",
        job.target,
    ...
    for (const job of failed)
      insert.run(
        ...
        "error",
        job.target,
        job.last_error ?? `${job.target} failed`,
        "{}",
        now,
        job.post_key,
        "target.failed",
...
function updateCredentialChecks(config: BackendConfig, backendDb: BackendDb): number {
  ...
  const now = new Date().toISOString();
  const statement =
    backendDb.sqlite.prepare(`INSERT INTO credential_checks(target,status,required_env_json,missing_env_json,last_checked_at,next_check_at,last_error,details_json)
