...
export function batchLikes(backendDb: BackendDb, postIds: string[], clientHash: string): Record<string, { likes: number; user_liked: boolean }> {
...
export function clientIpHash(c: Context, config: BackendConfig): string {
  ...
  return crypto.createHmac("sha256", config.LIKES_SALT || config.commandCenterToken || "alexgetman-likes").update(address).digest("hex");
}
...
export function recordPageview(backendDb: BackendDb, config: BackendConfig, rawPath: string): string {
  ...
  const bucket = days[day] ??= { total: 0, paths: {} };
  bucket.total += 1;
  ...
  const row = backendDb.sqlite.prepare(
  ...
  if (row?.post_key && row.target) {
    const existing = backendDb.sqlite.prepare("SELECT value FROM post_metrics WHERE post_key=? AND target=? AND metric_name='views'").get(row.post_key, row.target) as { value?: number } | undefined;
    const value = Number(existing?.value ?? 0) + 1;
    const rawJson = JSON.stringify({ path });
    backendDb.sqlite.prepare(`INSERT INTO post_metrics(post_key,target,metric_name,value,source,sampled_at,error,raw_json)
    ...
    backendDb.sqlite.prepare("INSERT INTO metric_samples(post_key,target,metric_name,value,sampled_at,source,raw_json) VALUES (?,?,'views',?,?,'site_pageview_endpoint',?)")
      .run(row.post_key, row.target, value, now.toISOString(), rawJson);
  }
  return path;
}
...
export function metricsSummary(config: BackendConfig): { total: number; today: number; last7: number; updated_at: unknown } {
  ...
  return { total: Number(data.total ?? 0), today: Number(days[keys[0] ?? ""]?.total ?? 0), last7: keys.slice(0, 7).reduce((sum, key) => sum + Number(days[key]?.total ?? 0), 0), updated_at: data.updated_at ?? null };
}

function normalizeMetricPath(value: string): string {
  let path = String(value || "/").split("#", 1)[0]!.split("?", 1)[0]!.trim();
...
function readMetricsFile(config: BackendConfig): Record<string, unknown> & { days: Record<string, { total: number; paths: Record<string, number> }> } {
  try {
    const data = JSON.parse(fs.readFileSync(config.SITE_METRICS_JSON, "utf8")) as Record<string, unknown>;
    return { ...data, days: data.days && typeof data.days === "object" ? data.days as Record<string, { total: number; paths: Record<string, number> }> : {} };
  } catch {
