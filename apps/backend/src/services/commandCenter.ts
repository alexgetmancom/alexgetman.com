...
export function postDebugPayload(backendDb: BackendDb, ref: string) {
  ...
  const targets = backendDb.sqlite.prepare("SELECT * FROM post_targets WHERE post_key=? ORDER BY target").all(postKey) as Record<string, unknown>[];
  const metrics = backendDb.sqlite.prepare("SELECT * FROM post_metrics WHERE post_key=? ORDER BY target, metric_name").all(postKey) as Record<string, unknown>[];
  const schedule = backendDb.sqlite.prepare("SELECT * FROM metric_schedule WHERE post_key=? ORDER BY target").all(postKey) as Record<string, unknown>[];
  const jobs = backendDb.sqlite.prepare("SELECT * FROM publish_jobs WHERE post_key=? OR post_id=? OR message_id=? ORDER BY job_id DESC").all(postKey, numericRef(ref), numericRef(ref)) as Record<string, unknown>[];
  return {
...
function resolvePostKey(backendDb: BackendDb, ref: string): string | null {
  ...
  const post = backendDb.sqlite.prepare("SELECT post_key FROM posts WHERE message_id=? OR post_key=?").get(id, `post:${id}`) as { post_key?: string } | undefined;
  if (post?.post_key) return post.post_key;
  const job = backendDb.sqlite.prepare("SELECT COALESCE(post_key, CASE WHEN post_id IS NOT NULL THEN 'post:' || post_id ELSE NULL END) AS post_key FROM publish_jobs WHERE message_id=? OR post_id=? ORDER BY job_id DESC LIMIT 1").get(id, id) as { post_key?: string | null } | undefined;
