...
function renderVideoChart(points: VideoTrendPoint[]): string {
  ...
  return `<div class="metric-chart video-chart"><div class="metric-chart__legend"><span><i style="background:var(--series-views)"></i>Просмотры</span><span><i style="background:var(--series-likes)"></i>Лайки</span><span><i style="background:var(--series-comments)"></i>Комментарии</span></div><svg viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" role="img" aria-label="Динамика метрик роликов"><line class="chart-grid" x1="${pad.x}" y1="${height / 2}" x2="${width - pad.x}" y2="${height / 2}"/><line class="chart-grid" x1="${pad.x}" y1="${height - pad.y}" x2="${width - pad.x}" y2="${height - pad.y}"/><polyline fill="none" stroke="var(--series-views)" stroke-width="2" class="chart-line" points="${line("views")}"/><polyline fill="none" stroke="var(--series-likes)" stroke-width="2" class="chart-line" points="${line("likes")}"/><polyline fill="none" stroke="var(--series-comments)" stroke-width="2" class="chart-line" points="${line("comments")}"/></svg><div class="video-chart-labels">${labels.map((point) => `<span>${escapeHtml(point.day.slice(5))}</span>`).join("")}</div></div>`;
}
