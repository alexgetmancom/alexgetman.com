...
export function renderOverviewSparkline(
  ...
  const average = points.reduce((total, point) => total + Math.max(0, point.value), 0) / points.length;
  const averageY = height - (average / max) * height;
  const bars = points
    .map((point, index) => {
      ...
      return `<g><rect x="${x.toFixed(2)}" y="${y.toFixed(2)}" width="${barWidth.toFixed(2)}" height="${barHeight.toFixed(2)}" rx="2" fill="${color}" opacity="${opacity.toFixed(2)}"/><rect class="chart-hit" x="${Math.max(0, x - barGap / 2).toFixed(2)}" y="0" width="${(barWidth + barGap).toFixed(2)}" height="${height}" data-tooltip="${tooltip}"/></g>`;
    })
    .join("");
