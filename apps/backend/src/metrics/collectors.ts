...
async function collectTelegram(task: MetricTask, config: BackendConfig, fetchImpl: typeof fetch): Promise<MetricResult> {
  ...
  const section = html.match(new RegExp(`data-post=["']${escaped}["'][\\s\\S]*?(?=data-post=["']${escapeRegExp(channel)}\\/|<\\/section>|$)`))?.[0];
...
async function collectThreads(task: MetricTask, config: BackendConfig, fetchImpl: typeof fetch): Promise<MetricResult> {
  const token = task.target === "threads_en" ? config.THREADS_EN_ACCESS_TOKEN ?? config.THREADS_ACCESS_TOKEN : config.THREADS_ACCESS_TOKEN;
...
async function collectFacebook(task: MetricTask, config: BackendConfig, fetchImpl: typeof fetch): Promise<MetricResult> {
  ...
  try {
    ...
    for (const [edge, name] of [["likes", "likes"], ["comments", "replies"]] as const) {
      try {
        const data = await requestJson<{ summary?: { total_count?: number } }>(fetchImpl, graphUrl(`${base}/${edge}`, token, { summary: "total_count", limit: "0" }));
  ...
  if (metrics.views == null) {
    for (const metric of ["fb_reels_total_plays", "total_video_views"]) {
      try {
        const data = await requestJson<{ data?: Array<{ name?: string; values?: Array<{ value?: number }> }> }>(fetchImpl, graphUrl(`${base}/video_insights`, token, { metric, ...(metric === "total_video_views" ? { period: "lifetime" } : {}) }));
...
async function collectDevto(task: MetricTask, config: BackendConfig, fetchImpl: typeof fetch): Promise<MetricResult> {
  ...
  if (config.DEVTO_API_KEY) {
    for (let page = 1; page <= 5 && !article; page += 1) {
      const data = await requestJson<DevtoMetrics[]>(fetchImpl, `https://dev.to/api/articles/me?per_page=100&page=${page}`, { headers: { "api-key": config.DEVTO_API_KEY, "User-Agent": "alexgetman-backend/1.0" } });
      article = data.find((item) => item.slug === slug || item.url?.replace(/\/$/, "") === task.url?.replace(/\/$/, ""));
      if (data.length === 0) break;
    }
    if (article) source = "devto_api_authenticated";
  }
  article ??= await requestJson<DevtoMetrics>(fetchImpl, `https://dev.to/api/articles/${username}/${slug}`, { headers: { "User-Agent": "alexgetman-backend/1.0" } });
  return {
    metrics: { views: Number(article.page_views_count ?? 0), likes: Number(article.public_reactions_count ?? article.positive_reactions_count ?? 0), replies: Number(article.comments_count ?? 0) },
    source,
    raw: { api_id: article.id, slug },
  };
}

type DevtoMetrics = { id?: number; slug?: string; url?: string; page_views_count?: number; public_reactions_count?: number; positive_reactions_count?: number; comments_count?: number };
...
async function collectBluesky(task: MetricTask, fetchImpl: typeof fetch): Promise<MetricResult> {
  ...
  const result = await requestJson<{ posts?: Array<{ uri?: string; likeCount?: number; replyCount?: number; repostCount?: number; quoteCount?: number }> }>(fetchImpl, `https://public.api.bsky.app/xrpc/app.bsky.feed.getPosts?${query}`);
...
async function collectMastodon(task: MetricTask, config: BackendConfig, fetchImpl: typeof fetch): Promise<MetricResult> {
  ...
  const parts = [];
  for (const id of ids) {
    const status = await requestJson<{ favourites_count?: number; replies_count?: number; reblogs_count?: number }>(fetchImpl, `${host}/api/v1/statuses/${id}`);
    const part = { id, likes: Number(status.favourites_count ?? 0), replies: Number(status.replies_count ?? 0), reposts: Number(status.reblogs_count ?? 0) };
    totals.likes += part.likes; totals.replies += part.replies; totals.reposts += part.reposts; parts.push(part);
  }
...
async function collectGitHub(task: MetricTask, config: BackendConfig, fetchImpl: typeof fetch): Promise<MetricResult> {
  ...
  const result = await requestJson<{ data?: { repository?: { discussion?: { comments?: { totalCount?: number }; reactions?: { totalCount?: number } } } } }>(fetchImpl, "https://api.github.com/graphql", {
    method: "POST",
    headers: { Authorization: `Bearer ${config.GITHUB_DISCUSSIONS_TOKEN}`, "Content-Type": "application/json", "User-Agent": "alexgetman-backend/1.0" },
    body: JSON.stringify({ query: "query($owner:String!,$repo:String!,$number:Int!){repository(owner:$owner,name:$repo){discussion(number:$number){comments{totalCount} reactions{totalCount}}}}", variables: { owner, repo, number: Number(number) } }),
  });
  const discussion = result.data?.repository?.discussion;
  return { metrics: { likes: Number(discussion?.reactions?.totalCount ?? 0), replies: Number(discussion?.comments?.totalCount ?? 0) }, source: "github_graphql", raw: { owner, repo, number } };
}

async function collectInstagramStory(task: MetricTask, config: BackendConfig, fetchImpl: typeof fetch): Promise<MetricResult> {
  const token = task.target === "instagram_stories_ru" ? config.INSTAGRAM_RU_ACCESS_TOKEN ?? config.INSTAGRAM_ACCESS_TOKEN : config.INSTAGRAM_EN_ACCESS_TOKEN ?? config.INSTAGRAM_ACCESS_TOKEN;
  ...
  const insights = await requestJson<{ data?: Array<{ name?: string; values?: Array<{ value?: number }> }> }>(fetchImpl, graphUrl(`https://${host}/${version}/${task.externalId}/insights`, token, { metric: "views,reach,replies,shares,total_interactions,navigation" }));
  const values = Object.fromEntries((insights.data ?? []).map((item) => [item.name ?? "", Number(item.values?.[0]?.value ?? 0)]));
  let likes = 0;
  try {
    likes = Number((await requestJson<{ like_count?: number }>(fetchImpl, graphUrl(`https://${host}/${version}/${task.externalId}`, token, { fields: "like_count,comments_count" }))).like_count ?? 0);
  } catch {
    // Story insights remain useful even when media fields have expired.
  }
  return { metrics: { views: values.views ?? values.reach ?? 0, reach: values.reach ?? 0, likes, replies: values.replies ?? 0, reposts: values.shares ?? 0, total_interactions: values.total_interactions ?? 0, navigation: values.navigation ?? 0 }, source: "instagram_graph_api", raw: insights };
}
...
async function collectTelegramStory(task: MetricTask, config: BackendConfig): Promise<MetricResult> {
  ...
  try {
    ...
    const result = await instance.invoke(new Api.stories.GetStoriesByID({ peer, id: [Number(task.externalId)] }));
    const story = result.stories[0];
    if (!(story instanceof Api.StoryItem)) throw new Error(`telegram_story_not_found:${task.externalId}`);
    const views = story.views instanceof Api.StoryViews ? story.views : null;
    const reactions = Number(views?.reactionsCount ?? 0);
    const forwards = Number(views?.forwardsCount ?? 0);
    return { metrics: { views: Number(views?.viewsCount ?? 0), likes: reactions, reposts: forwards, replies: 0, total_interactions: reactions + forwards }, source: "telegram_mtproto", raw: { story_id: task.externalId } };
  } finally {
...
async function collectX(task: MetricTask, config: BackendConfig, fetchImpl: typeof fetch): Promise<MetricResult> {
  ...
  const result = JSON.parse(body) as { data?: { public_metrics?: { impression_count?: number; like_count?: number; reply_count?: number; retweet_count?: number; quote_count?: number } } };
  const metrics = result.data?.public_metrics;
  return { metrics: { views: Number(metrics?.impression_count ?? 0), likes: Number(metrics?.like_count ?? 0), replies: Number(metrics?.reply_count ?? 0), reposts: Number(metrics?.retweet_count ?? 0), quotes: Number(metrics?.quote_count ?? 0) }, source: "x_api_v2", raw: result };
}
...
async function collectLinkedIn(task: MetricTask, config: BackendConfig, fetchImpl: typeof fetch): Promise<MetricResult> {
  ...
  const response = await requestJson<{ likesSummary?: { totalLikes?: number }; commentsSummary?: { totalFirstLevelComments?: number } }>(fetchImpl, `https://api.linkedin.com/rest/socialActions/${encodeURIComponent(task.externalId)}`, {
    headers: { Authorization: `Bearer ${config.LINKEDIN_ACCESS_TOKEN}`, "Linkedin-Version": config.LINKEDIN_API_VERSION, "X-Restli-Protocol-Version": "2.0.0" },
  });
  return { metrics: { likes: Number(response.likesSummary?.totalLikes ?? 0), replies: Number(response.commentsSummary?.totalFirstLevelComments ?? 0) }, source: "linkedin_social_actions", raw: response };
}
