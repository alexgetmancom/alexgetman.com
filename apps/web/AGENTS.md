...
**Never write a raw colour in CSS** — only `var(--*)`. The resolved theme is `data-theme` on
`<html>`, set by an inline script before first paint; `data-theme-mode` beside it holds which of
`system` (the default, stores nothing), `light` or `dark` the reader picked.
