...
caller. Anything that exists to coordinate a team, or to keep someone else's callers working, is
dead weight. Assume the direct version of the work and do not ask permission for it.

- No transitional scaffolding: no shims, aliases kept "for one release", facade or adapter layers,
  dual read/write windows, flags guarding an old path, or backfills for a shape being abandoned.
  Cut over in one move — rename, delete the old path in the same commit, update every call site,
  migrate or drop the data.
...
- A shared abstraction that needs a branch on which caller it serves is the wrong abstraction. Push
  the difference into an explicit capability, or keep the implementations apart under names that say
  what they do. Never add the branch.
- Deleting beats adding. Say how many lines went.
- One concept, one name. Two names for the same thing is a defect.
- Finish in one move: no TODO breadcrumbs, no stubs "for later", no half-migrated state. If it
  cannot be finished, say so instead of leaving a seam.
...
There are two Studios in production, one container each, and they are separate systems with
separate databases, channels, and post numbering:

- **alex** — the full Studio: bilingual site, text and video, seven delivery targets. The default.
- **maru** — video-only, publishing to its own Telegram channel. Text posting is off, so
  `expected targets: telegram` and a small `recent` list are correct, not a gap; its video pipeline
  lives in the `video*` sections of `status` and `audit`.

`bun run ops:prod [--as alex|maru] <command>` is the only production route; without `--as` it is
`alex`, and every run prints the deployment it resolved to. The names are the table at the top of
`scripts/ops-prod.ts`. A request that names Maru means `--as maru` throughout, including the
follow-up commands; one that names neither means alex. `.env.local` holds only `OPS_SSH_TARGET`.
...
with `bun run ops:prod guide --json`, because a container runs its last deployed revision and a
command committed but not yet deployed comes back as `unknown command`. `guide.catalog` says which
case you are in. Inside a container there is no SSH configuration, so production `guide` reports
`production.configured: false` and `route: local` — that is the container describing itself, not a
broken setup.

For a missing publication start with `audit`; it covers both the text and video pipelines. When the
...
`find --query "text"` resolves a ref when the post is older. Refs are `post:N`; only
`publication-repair` also takes `video:N`. Never open the production database by hand. Get CLI
output before reading source for production state.
...
path is the right argument. `--sampled-at` stamps the metric history, so it is the export's own
timestamp — the file's mtime, never `now`. A byte-identical file is a no-op by SHA-256, so a repeat
costs nothing; a re-export of the same period is a new file and a new sample.
...
rule changes, because a byte-identical CSV will not re-link anything.
...
`COMMAND_CENTER_TOKEN` matches the token in the URL (`.claude/launch.json` sets `dev`). An empty
database is normal, not a bug — never write INSERTs by hand. Change fixture shape in
`apps/web/src/server/site-fixture.ts` (public read model) or
`apps/web/src/server/dashboard-fixture.ts` (Command Center), not in the seed script. The dashboard
fixture is deliberately not all-green, and audience counts read `—` locally because they come from
live platform APIs.
