...
**Токены Threads протухают.** Долгоживущий токен Threads истекает через 60 дней
после выпуска, и никто здесь его не продлевает — публикация просто начинает
падать. Подключение Threads через провайдера снимает эту проблему.
...
**Telegram Stories нужна пользовательская сессия**, а не бот:
`TELEGRAM_CHANNEL_STORIES_SESSION` — это сессия MTProto, и помощника для её
получения пока нет.
