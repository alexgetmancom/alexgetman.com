...
- Старый домен `ialexey.ru` retired: без 301/302 на `alexgetman.com`; если домен ещё указывает на VPS, он должен отдавать `410 Gone` + `noindex`.
...
- Пост через `@alexgetman_bot` является owner-approved input для pipeline; публичный canonical source всегда `alexgetman.com`.
- Telegram channel не обязателен.
...
- Текущие production targets и platform limits ведутся в private `alexgetman-posting/docs/platform-limits.md`.
...
- Изменено: Настроено автоматическое удаление первого эмоджи для Facebook и LinkedIn в `crosspost_worker.py`.
