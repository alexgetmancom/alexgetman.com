...
| --- | --- | --- |
| 48. Intelligent Registry-based deploy (ВЫСШИЙ ПРИОРИТЕТ) | GitHub Actions пушит образ в GHCR и триггерит деплой. Скрипт на сервере проверяет здоровье `/readyz` нового контейнера. В случае сбоя — авто-откат на стабильный хэш. Бот шлет статус в Telegram с инлайн-кнопкой «Откатить» для ручного отката в 1 клик. | Частично: GHCR digest -> GitHub deploy-job -> VPS agent -> `/readyz` прошёл end-to-end; перед activation CI атомарно синхронизирует и валидирует compose manifest. Предыдущий digest хранится для rollback. Нужна проверка нажатия Telegram-кнопки rollback. |
...
| 10. Shared media cache | Один download/transcode/stage на пост и локаль, без параллельных ffmpeg и дублирующих файлов. | Частично: SSR строит URL из published media manifest; site worker materializes RU/EN files, включая восстановленные production jobs. Нужен audit дедупликации всех target uploads. |
...
| 35. SQLite parity and safety | WAL, busy timeout, fixture compatibility и production-readonly schema audit подтверждены. | Частично: WAL/busy timeout, fixture и production-readonly audit выполнены; остаётся автоматизировать schema report и concurrency stress на production copy. |
...
| 39. Web canonical parity | EN root, RU `/ru`, canonical URLs, feeds, sitemap и JSON-LD не используют Telegram как source. | Частично: главная, RU и feed/Markdown/search routes теперь SSR и читают SQLite; нужно проверить canonical/JSON-LD по всем post/category URLs. |
...
| 50. Code quality gates | Biome, Knip, staged hook, typecheck, unit/integration tests, Docker smoke test и browser checks обязательны в CI. | Частично: Lefthook, Biome, Knip, typecheck, Bun tests и SSR build зелёные; production Chromium screenshot подтвердил media/cards. Нужно добавить browser smoke как обязательный CI job и Docker smoke. |
