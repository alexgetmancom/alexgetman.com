...
- **Возможное расхождение между репозиторным и активным Nginx-контуром.** Во время восстановления публикации Maru появилось наблюдение: CI/CD записывает конфиг из `deploy/nginx/alexgetman.com.conf.example` в `/etc/nginx/sites-available/alexgetman.com`, тогда как фактический трафик `alexgetman.com`, вероятно, проходит через исторические активные конфиги в `sites-enabled` (внешний TLS → внутренний порт 81). Из-за этого новые `location`-маршруты могут не попадать в реально работающий vhost. Это нужно отдельно подтвердить полной картой include/symlink/портов и ownership конфигов, а затем выбрать безопасный единый deploy-контур — например, версионируемый include для служебных маршрутов Maru. Активный конфиг Alex целиком не заменять до подтверждения паритета маршрутов.
